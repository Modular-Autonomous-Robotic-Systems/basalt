...
void LocalMapper::MapLocally() {
  ...
  while (!mpStopLocalMapping) {
    ...
    CollectNewKeyframesAfterMatching();  // promote to mpLatestKeyframesMatches
...
void LocalMapper::MatchLocal() {
  ...
  auto compute_pairs = [&](const tbb::blocked_range<size_t>& r) {
    for (size_t i = r.begin(); i != r.end(); ++i) {
      ...
      // Only query from new KFs — this is the key difference from match_all.
  ...
  auto match_func = [&](const tbb::blocked_range<size_t>& r) {
    ...
    for (size_t j = r.begin(); j != r.end(); ++j) {
      ...
      if (static_cast<int>(md.matches.size()) > config.mapper_min_matches) {
        matched++;
  ...
  for (const auto& kv : feature_tracks) {
    ...
    if (!lmdb.landmarkExists(kv.first)) {
      ...
      for (auto it_o = std::next(it_h); it_o != kv.second.end(); ++it_o) {
        ...
        if (T_h_o.translation().squaredNorm() < min_triang_dist2) continue;
...
size_t LocalMapper::ComputeCovisibility(int64_t tid_a, int64_t tid_b,
  ...
  for (int i = 0; i < num_cameras; i++) {
    for (int j = 0; j < num_cameras; j++) {
      covisibility += lmdb.getObservationsCountForPair(TimeCamId(tid_a, i),
                                                       TimeCamId(tid_b, j));
      covisibility += lmdb.getObservationsCountForPair(TimeCamId(tid_b, j),
                                                       TimeCamId(tid_a, i));
    }
  }
  return covisibility;
